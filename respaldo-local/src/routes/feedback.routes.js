...
router.post('/admin/seguimiento/:id', checkAuth, upload.single('imagen'), async (req, res) => {
  try {
    ...
    await feedbackManager.updateFeedback(req.params.id, {
      seguimientoImagen: imageUrl
    });
    res.redirect('/admin');
  } catch (err) {
    res.status(500).send('❌ Error al guardar seguimiento: ' + err.message);
  }
});
